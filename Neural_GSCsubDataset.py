...
def GetTargetVector(filePath):  #reading the entire target values into a numpy vector
    data = pd.read_csv(filePath)
    t=[]
    t=data['target'].values  # as a numpy array
    print(np.shape(t))
    return t

def GetRawData(filePath):   #reading the entire input features into numpy matrix
    data = pd.read_csv(filePath)
    data = data.loc[:, (data != 0).any(axis=0)] #removing any zero columns
    t=[]
...
RawTarget = GetTargetVector('GSCDatasetwithFeatureSubtraction.csv')
RawData = GetRawData('GSCDatasetwithFeatureSubtraction.csv')
...
num_epochs = 5000           #no. of times all of the training data is used to train the weights
...
for i,j in zip(TestingData,TestingTarget):   #iterating over the test input features and actual labels
    y = model.predict(np.array(i).reshape(-1,509)) #Generates output predictions for the input samples; reshape converts the input into a column matrix
    t=np.around(y)
