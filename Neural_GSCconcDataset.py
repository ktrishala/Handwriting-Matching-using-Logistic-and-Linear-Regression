...
def GetTargetVector(filePath):   #reading the entire target values into numpy vector
...
def GenerateTrainingTarget(RawTarget,TrainingPercent):
    ...
    print('Training Target shape',t.shape)
    return t
...
def GenerateTrainingDataMatrix(RawData, TrainingPercent):
    ...
    d2 = RawData[:T_len]
    print('Training Data shape',d2.shape)
    return d2
...
def GenerateTestingTarget(RawTarget,TestingPercent):
    ...
    t           = RawTarget[-T_len:]
    print('Testing Target Shape',t.shape)
    return t
...
def GenerateTestingDataMatrix(RawData, TestingPercent):
    ...
    print (T_len)
    d2 = RawData[-T_len:]
    print('Testing Data Shape',d2.shape)
    return d2
...
RawTarget = GetTargetVector('GSCDatasetwithFeatureConcatenation.csv')
RawData = GetRawData('GSCDatasetwithFeatureConcatenation.csv')
...
def get_model():
    ...
    model.add(Dense(fourth_dense_layer_nodes))
    model.add(Activation('relu'))
...
num_epochs = 50           #no. of times all of the training data is used to train the weights
...
for i,j in zip(TestingData,TestingTarget):   #iterating over the test input features and actual labels
    y = model.predict(np.array(i).reshape(-1,1017)) #Generates output predictions for the input samples; reshape converts the input into a column matrix
    t=np.around(y)
