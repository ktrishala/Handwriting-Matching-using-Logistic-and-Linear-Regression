...
def GetTargetVector(filePath):  #generating target vector
...
def GetRawData(filePath):        #generating input features matrix
    data = pd.read_csv(filePath)
    t=[]
...
def GenerateTestingTarget(RawTarget,TestingPercent):
    T_len = int(math.ceil(len(RawTarget)*(TestingPercent*0.01))) #Taking only 20% of the Target data for training
    t           = RawTarget[-T_len:]
    print(t.shape)
    return t

def GenerateTestingDataMatrix(RawData, TestingPercent):
    T_len = int(math.ceil(len(RawData)*0.01*TestingPercent)) #Taking only 20% of the data matrix for training
    print (T_len)
    d2 = RawData[-T_len:]
    print(d2.shape)
    return d2
...
def get_model():
    ...
    model.compile(optimizers.Adam(lr=0.000005),   #Adam is a type of Gradient descent optimization algorithms
...
for i,j in zip(TestingData,TestingTarget):   #iterating over the test input features and actual labels
    y = model.predict(np.array(i).reshape(-1,9)) #Generates output predictions for the input samples; reshape converts the input into a column matrix
    t=np.around(y)
