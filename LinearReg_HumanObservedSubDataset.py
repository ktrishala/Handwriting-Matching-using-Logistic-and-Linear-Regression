...
def GetTargetVector(filePath): #creating the target vector of the entire dataset
...
def GenerateValTargetVector(rawData, ValPercent, TrainingCount): 
    valSize = int(math.ceil(len(rawData)*ValPercent*0.01)) #Taking only 10% of the Target data for validation
    V_End = TrainingCount + valSize
...
def GetErms(VAL_TEST_OUT,ValDataAct):  #calculating Erms to compute accuracy and error
    sum = 0.0
    t=0
    accuracy = 0.0
    counter = 0
